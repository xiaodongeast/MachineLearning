...
public class Diabetes {
	...
	public static void main(String[] args) {
		...
		double[] temperature= { 0.95, 0.65, 0.45, 0.1};
	 int[] inter = {2000};
		//int[] inter = {2,5,10,20,40,80,100,200,500,1000};
		for (int i=0;i<temperature.length;i++)
			
		{ System.out.println("$$$$$$$$$$$$$$$$$cooling "+temperature[i]);
			oa[0] = new SimulatedAnnealing(1E11, temperature[i], nnop[1]);
		...
		{ System.out.println("################# iterations  " +inter[j]  );
			...
			trainingTime= buildModel( oa[0], oaNames[0], networks[0],inter[j]);
			testResult= testModel(trainData,networks[0]  );
			finalTrainTime=finalTrainTime+trainingTime;
			finalTrainScore=finalTrainScore+(testResult[0]/(testResult[0]+testResult[1])*100);
		 
		    String results;
			results=  "Training,"+ "SA," +"cooling, "+temperature[i] + "iteration "+ inter[j]+  ",Correctly classified, " + testResult[0] +
		...
		//	System.out.println(results);
			...
			testResult= testModel(test,networks[0]  );
			
			results=  " Predicting, "+ "SA," +"cooling, "+temperature[i] + "iteration "+ inter[j]+ ",Correctly classified,+ "+ testResult[0] + 
					",Incorrectly classified, " + testResult[1] + " ,Percent correctly classified:, "
					+ df.format(testResult[0]/(testResult[0]+testResult[1])*100) + "%Training time: " + df.format(trainingTime)
					+ ",Testing time: " + df.format(testResult[2])+"\n";
		//	System.out.println(results);
			...
			//	System.out.println(results);
			 ...
			 finalResult=finalResult+"SA,"+"cooling,  "+temperature[i]+ ",  iteration," +inter[j] +",  average trainScore,"+finalTrainScore+ ",averageTestScore,"+finalTestScore+
		...
		write_output_to_file(finalResult,"diabetesSAResultfinal.csv",true);
