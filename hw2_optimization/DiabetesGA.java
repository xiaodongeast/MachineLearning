...
public class DiabetesGA {
	...
	int[] inter = {2,5,10,100,200,500,1000};
	//	int[] inter = {500};
		int[] populationSizes={200,100,50};
		//int[] populationSizes={100};
		//double[] geneticRates= { 0.5};
		double[] geneticRates= { 0.5};
		...
		for (int population : populationSizes)

		{ 
			System.out.println("$$$$$$$$$$$$$$$$$ population "+population);

			for( double rate: geneticRates)
			{  
				System.out.println("$$$$$$$$$$$$$$$$$ genetic rate "+rate);
				int mate=(int) (population*rate);
				int mutate=(int)(population*rate*rate);
				...
				{ 	
					...
					{ 
						...
						trainingTime= buildModel( oa[2], "GA", networks[2],inter[j]);
						testResult= testModel(trainData,networks[2]  );
						finalTrainTime=finalTrainTime+trainingTime;
						finalTrainScore=finalTrainScore+(testResult[0]/(testResult[0]+testResult[1])*100);
						String results;
						results=  "Training,"+ "GA," +"population, "+population + ",mate, "+mate+",mutate," +mutate+ ",iterateion,"+ inter[j]+  ",Correctly classified, " + testResult[0] +
						...
						testResult= testModel(test,networks[2]  );

						results=  "Predicting,"+ "GA," +"population, "+population + ",mate, "+mate+",mutate," +mutate+ ",iterateion,"+ inter[j]+  ",Correctly classified, " + testResult[0] +
								",Incorrectly classified, " + testResult[1] + " ,Percent correctly classified:, "
								+ df.format(testResult[0]/(testResult[0]+testResult[1])*100) + ",%Training time:, " + df.format(trainingTime)
								+ ",Testing time:, " + df.format(testResult[2])+"\n";
						//	System.out.println(results);
					...
					finalResult=finalResult+"GA,"+"population,  "+population+ ",mate, "+mate+",mutate," +mutate+ ",iteration," +inter[j] +",  average trainScore,"+finalTrainScore+ ",averageTestScore,"+finalTestScore+
		...
		write_output_to_file(finalResult,"diabetesGAResultCorrectOnePoint500.csv",true);
